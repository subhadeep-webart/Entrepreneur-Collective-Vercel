...
import RegisterAsModal from "../../Modals/component/RegisterAsModal";

const FormFooter = ({ isLoginPage = true }) => {
  const headerText = isLoginPage
    ? `Don't have an account?`
    : `Already have an account?`;
  // const href = isLoginPage ? "/registration" : "/login";
  const linkText = isLoginPage ? "Register as" : "Login";
    ...
    <>
      <h2 className="text-center font-normal text-base flex items-center gap-2">
        ...
        {isLoginPage ? (
          ...
          <Link
            href="/login"
            className={"text-link w-fit font-bold text-sm bg-transparent"}
          >
            {linkText}
          </Link>
        )}
      ...
      <div className="flex justify-start items-center gap-2">
        ...
        <p className="text-form-text font-normal text-sm">{googleText}</p>
      </div>
    </>
  );
};
